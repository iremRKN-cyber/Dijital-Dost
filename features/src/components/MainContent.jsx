...
export default function MainContent() {
  ...
  // 1. ZIRH: CRASH HATASI DÜZELTİLDİ
  ...
  async function fetchDynamicEduContent(type) {
    ...
    try {
      ...
      if (type === 'scenarios') {
        const prompt = `Şu an internette sıkça rastlanan, BİRBİRİNDEN ÇOK FARKLI 2 güncel siber dolandırıcılık veya zorbalık senaryosu üret. SADECE JSON dizisi dön:
  ...
  async function analyzeWithGemini() {
    try {
      ...
      console.error("Gemini Hatası (B Planı Devrede):", err)

      // 2. ZIRH: BLUE TEAM FALLBACK (B PLANI) BURADA DEVREYE GİRİYOR!
      return {
        durum: "tehlikeli",
        mesaj: "🚨 SİSTEM MEŞGUL - KORUMA KALKANI AKTİF: \n\nAPI kotası dolduğu için 'Fallback' (Acil Durum) moduna geçtik. Bu mesaj veya görsel siber güvenlik filtrelerimizce RİSKLİ bulunmuştur. Lütfen içindeki linklere kesinlikle TIKLAMAYIN ve kişisel bilgilerinizi paylaşmayın.",
        nedenler: mode === 'child' ? ["Sistem otomatik güvenlik kalkanını devreye soktu.", "Kaynağı belirsiz linkler ve mesajlar tehlikelidir."] : []
      }
    }
  }
            ...
            <div className="p-8 sm:p-12 min-h-[400px] relative">
              ...
              {eduTab === 'scenarios' && (
                <div className="space-y-6 animate-in slide-in-from-left-4">
                  <h3 className="text-3xl font-black text-indigo-900 mb-8 flex items-center gap-3"><span className="p-2 bg-indigo-100 rounded-lg">🎯</span> Canlı Senaryo Radarı</h3>
                  ...
                  <button onClick={() => fetchDynamicEduContent('scenarios')} disabled={isEduLoading} className="w-full sm:w-auto bg-slate-100 text-indigo-700 font-black py-4 px-8 rounded-2xl hover:bg-indigo-50 transition-colors mt-6 flex items-center justify-center gap-3 text-lg"><span className="animate-spin-slow">🔄</span> YZ ile Yeni Senaryolar Üret</button>
                </div>
              )}
              {eduTab === 'laws' && (
                <div className="space-y-6 animate-in slide-in-from-bottom-4">
                  <h3 className="text-3xl font-black text-indigo-900 mb-8 flex items-center gap-3"><span className="p-2 bg-indigo-100 rounded-lg">⚖️</span> Siber Haklarımız</h3>
                  ...
                  <button onClick={() => fetchDynamicEduContent('laws')} disabled={isEduLoading} className="w-full sm:w-auto bg-slate-100 text-indigo-700 font-black py-4 px-8 rounded-2xl hover:bg-indigo-50 transition-colors mt-6 flex items-center justify-center gap-3 text-lg"><span className="animate-spin-slow">🔄</span> YZ ile Yeni Bilgiler Üret</button>
                </div>
              )}
              {eduTab === 'quiz' && (
              ...
              <div className="space-y-6 relative z-10">
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    } catch (e) { setResultMessage(e.message) } finally { setIsLoading(false) }
                  }}
                ...
                >
                  {isLoading ? <><span className="animate-spin text-3xl">⚙️</span> YAPAY ZEKA İNCELİYOR...</> : <>ŞİMDİ İNCELE <span className="text-3xl">🚀</span></>}
                </button>
              </div>
              <div className="mt-12 relative z-10">
                {autoNotified && (
                  <div className="mb-8 bg-red-600 text-white p-8 rounded-[2rem] shadow-2xl border-4 border-red-500 animate-in fade-in slide-in-from-top-4">
                    ...
                    <button onClick={() => window.open(`https://wa.me/?text=${encodeURIComponent(`🚨 ACİL DURUM: Dijital Dost, çocuğunuzun cihazında tehlikeli bir mesaj tespit etti!\n\n*İncelenen:* "${text}"\n*Analiz:* ${resultMessage}`)}`, '_blank')} className="w-full bg-white text-red-700 font-black py-5 rounded-2xl text-2xl hover:bg-red-50 transition-all shadow-lg flex items-center justify-center gap-3">📱 AİLEME HEMEN HABER VER</button>
                  </div>
                )}
          ...
          <aside className="lg:col-span-1">
            <div className={`p-8 rounded-[2.5rem] shadow-xl border border-slate-100 ${isChild ? 'bg-gradient-to-b from-emerald-50 to-white' : 'bg-gradient-to-b from-sky-50 to-white'} h-full`}>
              ...
              <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm relative overflow-hidden min-h-[160px] flex items-center justify-center text-center">
                ...
                <p key={radarIndex} className="text-slate-700 text-lg font-bold animate-in fade-in zoom-in-95 duration-500">{currentRadarText}</p>
              </div>
              <p className="text-center text-slate-400 font-medium text-sm mt-6 flex items-center justify-center gap-2"><span className="animate-spin-slow">📡</span> Bölge taranıyor...</p>
            </div>
          </aside>
        </div>
      </div>
